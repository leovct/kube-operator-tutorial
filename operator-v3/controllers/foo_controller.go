...
func (r *FooReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.List(ctx, &podList); err != nil {
		...
		for _, item := range podList.Items {
			if item.GetName() == foo.Spec.Name {
				log.V(1).Info("pod linked to a foo custom resource found", "name", item.GetName())
				friendFound = true
			}
		}
	}
	...
	log.V(1).Info("foo's happy status updated", "status", friendFound)
	...
	if err := r.Client.List(context.TODO(), &list); err != nil {
		...
		for _, item := range list.Items {
			if item.Spec.Name == obj.GetName() {
				...
				log.V(1).Info("pod linked to a foo custom resource issued an event", "name", obj.GetName())
			}
		}
	}
	return req
